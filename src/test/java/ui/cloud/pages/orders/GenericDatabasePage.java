...
public class GenericDatabasePage extends IProductPage {
    ...
    public GenericDatabasePage(GenericDatabase product) {
        super(product);
    }

    @Override
    protected void checkPowerStatus(String expectedStatus) {
        new GenericDatabasePage.VirtualMachineTable(POWER).checkPowerStatus(expectedStatus);
    }

    public void start() {
        checkPowerStatus(GenericDatabasePage.VirtualMachineTable.POWER_STATUS_OFF);
        runActionWithoutParameters(BLOCK_APP, "Включить");
        checkPowerStatus(GenericDatabasePage.VirtualMachineTable.POWER_STATUS_ON);
    }
    ...
    public void delete() {
        ...
        new GenericDatabasePage.VirtualMachineTable(POWER).checkPowerStatus(GenericDatabasePage.VirtualMachineTable.POWER_STATUS_DELETED);
    }

    public void restart() {
        new GenericDatabasePage.VirtualMachineTable(POWER).checkPowerStatus(GenericDatabasePage.VirtualMachineTable.POWER_STATUS_ON);
    ...
    public void addGroup(String role, List<String> groups) {
        ...
        groups.forEach(group -> Assertions.assertTrue(new GenericDatabasePage.RoleTable().getGroupsRole(role).contains(group), "Не найдена группа " + group));
    }

    @Step("Изменить состав групп у роли {role} на {groups}")
    public void updateGroup(String role, List<String> groups) {
        checkPowerStatus(VirtualMachine.POWER_STATUS_ON);
        runActionWithParameters(new GenericDatabasePage.RoleTable().getRoleMenuElement(role), "Изменить состав группы", "Подтвердить", () -> {
    ...
    public void deleteGroup(String role) {
        checkPowerStatus(VirtualMachine.POWER_STATUS_ON);
        runActionWithoutParameters(new GenericDatabasePage.RoleTable().getRoleMenuElement(role), "Удалить группу доступа");
        Assertions.assertThrows(NotFoundException.class, () -> new GenericDatabasePage.RoleTable().getRoleRow(role));
    }
