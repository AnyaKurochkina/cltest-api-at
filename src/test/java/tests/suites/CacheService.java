...
public class CacheService {
    ...
    public static synchronized void loadEntities(String file) {
        try {
            if (Files.exists(Paths.get(file))) {
                ...
                entities = new ObjectMapper().readValue(fileInputStream, ArrayList.class);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void saveEntity(Class<?> c, Entity e) {
        e.setClassName(c.getName());
        String serialize = new Gson().toJson(e, c);
        entities.add(serialize);
    }
    ...
    public class EntityObject {
        ...
        public <T extends Entity> T getEntity() {
            for (String shareDataElement : entities) {
