...
public class PostgresSQLCluster extends IProduct {
    ...
    public void createDb(String dbName) {
        ...
        if (getEnv().equalsIgnoreCase("LT"))
            OrderServiceSteps.executeAction("postgresql_cluster_create_db", this, new JSONObject(String.format("{conn_limit: -1, db_name: \"%s\", db_admin_pass: \"%s\"}", dbName, adminPassword)), this.getProjectId());
        else
            OrderServiceSteps.executeAction("postgresql_cluster_create_db_nonprod", this, new JSONObject(String.format("{conn_limit: -1, db_name: \"%s\", db_admin_pass: \"%s\"}", dbName, adminPassword)), this.getProjectId());
